...
- Use `uv` for development. Run tests with `uv run python -m unittest discover -s tests`. Run the CLI with `uv run photo-archivist --source onedrive|<path>`. Face commands: `label-face`, `backfill-crops`, `serve-faces`, `train-faces`. Add dependencies in `pyproject.toml`.
