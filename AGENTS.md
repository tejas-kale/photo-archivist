...
- CLI orchestration lives in `archive.py`. Source adapters live in `sources/`. Vision inference lives in `describe.py`, CLIP embeddings in `embed.py`, SQLite persistence in `store.py`, sidecars in `sidecar.py`, and original-opening helpers in `open_original.py`.
...
- Treat source selection, Photos library access, OneDrive path handling, image paths, Ollama/mlx-vlm requests, embeddings, SQLite rows, sidecars, and open-original behaviour as behaviours that need tests when touched.
