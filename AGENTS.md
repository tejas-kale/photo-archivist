...
- Use `uv` for development. Run tests with `uv run python -m unittest discover -s tests`. Run the CLI with `uv run photo-archivist --source onedrive|<path>`. Add dependencies in `pyproject.toml`.
